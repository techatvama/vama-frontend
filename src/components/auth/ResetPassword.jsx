...
export default function ResetPassword() {
    ...
    useEffect(() => {
        ...
        api.get('/auth/validate-token', { params: { token, purpose: 'password_reset' } })
    ...
    if (checking) {
        return <AuthShell title="One moment…" subtitle="Verifying your link">
            <div className="flex justify-center py-12"><Loader2 className="animate-spin" style={{ color: BRAND.purple }} size={36} /></div>
        </AuthShell>;
    }

    if (!valid && !done) {
        return <AuthShell title="Link expired" subtitle="This reset link is invalid or used">
            <div className="bg-red-50 border border-red-100 text-red-600 p-4 rounded-xl flex items-center gap-2.5 text-sm font-medium">
                <AlertCircle size={18} className="shrink-0" /> This reset link is no longer valid. Request a new one.
            </div>
            <button onClick={() => navigate('/forgot-password')} className="mt-6 w-full text-white rounded-xl py-3.5 font-semibold shadow-lg transition-opacity hover:opacity-90" style={{ backgroundColor: BRAND.orange }}>
                Request a new link
            </button>
        </AuthShell>;
    }
    ...
    if (done) {
        return <AuthShell title="Password updated" subtitle="You're good to go" icon={<CheckCircle2 size={22} className="text-emerald-600" />}>
            <div className="bg-emerald-50 border border-emerald-100 text-emerald-700 p-4 rounded-xl flex items-center gap-2.5 text-sm font-medium">
                <CheckCircle2 size={18} className="shrink-0" /> Your password has been changed. Sign in with your new password.
            </div>
    ...
    return (
        <AuthShell title="Choose a new password" subtitle="Reset your VAMA password" icon={<KeyRound size={22} />}>
