...
export default function ResetPassword() {
    ...
    if (!valid && !done) {
        return <AuthShell title="Link expired" subtitle="This reset link is invalid or used">
            <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-5 rounded-2xl flex items-center gap-3 text-sm font-bold">
                <AlertCircle size={20} /> This reset link is no longer valid. Request a new one.
            </div>
            <button onClick={() => navigate('/forgot-password')} className="mt-6 w-full bg-indigo-500 hover:bg-indigo-600 text-white rounded-[28px] py-5 font-black transition-all">
                Request a new link
            </button>
        </AuthShell>;
    }
    ...
    if (done) {
        return <AuthShell title="Password updated" subtitle="You're good to go" icon={<CheckCircle2 className="text-emerald-400" />}>
            <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 p-5 rounded-2xl flex items-center gap-3 text-sm font-bold">
                <CheckCircle2 size={20} /> Your password has been changed. Sign in with your new password.
            </div>
    ...
    return (
        <AuthShell title="Choose a new password" subtitle="Reset your VAMA password" icon={<KeyRound className="text-yellow-400" />}>
